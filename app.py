# -*- coding: utf-8 -*-
"""
Elliott Waves Streamlit – v3 (z mocniejszym pobieraniem danych Yahoo + debug)
"""
...
def _flatten_yf_cols(df: pd.DataFrame) -> pd.DataFrame:
    if isinstance(df.columns, pd.MultiIndex):
        try:
            ...
            # Jeśli któryś poziom zawiera OHLC – wybierz ten poziom
            if set0 & wanted and not (set1 & wanted):
                df.columns = lvl0
            elif set1 & wanted and not (set0 & wanted):
                df.columns = lvl1
            else:
                # fallback: złącz poziomy spacją
...
def _yf_fetch_debug(ticker: str, period: str) -> Tuple[pd.DataFrame, list]:
    ...
    # Attempt 1 – download (classic)
    try:
        d1 = yf.download(ticker, period=period, interval="1d",
    ...
    # Attempt 2 – history() if still empty
    if df.empty:
        try:
    ...
    # Attempt 3 – download with auto_adjust=True
    if df.empty:
        try:
            d3 = yf.download(ticker, period=period, interval="1d",
    ...
    # Attempt 4 – compute start date from period
    if df.empty:
        try:
            days = 365
            ...
            d4 = yf.download(ticker, start=str(start), interval="1d",
                             auto_adjust=True, progress=False, group_by='column', threads=False)
...
def _wave_line(fig: go.Figure, seq: List[Pivot], name: str) -> None:
    xs = [p.ts for p in seq]; ys = [p.price for p in seq]
    fig.add_trace(go.Scatter(x=xs, y=ys, mode="lines+markers", name=name))

def _pattern_table(seq: List[Pivot], labels: List[str]) -> pd.DataFrame:
    return pd.DataFrame([{"Fala": lab, "Data": p.ts, "Cena": p.price, "Idx": p.i, "Typ": p.kind} for p,lab in zip(seq, labels)])
...
with st.sidebar:
    ...
    choose_pattern = st.multiselect("Szukanie wzorców", options=["Impuls 1-2-3-4-5","Korekta ABC"],
                                    default=["Impuls 1-2-3-4-5","Korekta ABC"])
    min_bars_wave = st.slider("Min. świece na falę", 1, 30, 3, 1)
    fib_tol = st.slider("Tolerancja Fibo (±)", 0.05, 0.8, 0.35, 0.05)
    overlap_tol = st.slider("Tolerancja nakładania 4 na 1", 0.0, 0.1, 0.02, 0.005)
    st.markdown("---")
...
if source == "Yahoo Finance":
    ...
    file = st.file_uploader("Wgraj CSV z kolumnami: Date, Open, High, Low, Close, Volume (nagłówki nie są obowiązkowe).", type=["csv"])
    if file is not None:
        try:
...
if data_df is not None and not data_df.empty:
    ...
    pivots = zigzag_from_close(data_df, close_col="Close",
                               pct_threshold=pct_threshold, min_bars_between_pivots=min_bars_between,
    ...
    if "Impuls 1-2-3-4-5" in choose_pattern:
        best_impulse = detect_impulse(pivots, direction=None,
    ...
    if show_table:
        with st.expander("Punkty ZigZag"):
            st.dataframe(pd.DataFrame([{"Idx": p.i, "Data": p.ts, "Cena": p.price, "Typ": p.kind} for p in pivots]), use_container_width=True)
        if best_impulse is not None:
            with st.expander("Szczegóły dopasowania: Impuls 1-2-3-4-5"):
                st.dataframe(_pattern_table(best_impulse.pivots[1:], ["1","2","3","4","5"]), use_container_width=True)
                st.json(best_impulse.details)
        if best_abc is not None:
            with st.expander("Szczegóły dopasowania: Korekta ABC"):
                st.dataframe(_pattern_table(best_abc.pivots[1:], ["A","B","C"]), use_container_width=True)
